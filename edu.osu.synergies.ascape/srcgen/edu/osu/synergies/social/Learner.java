 ...
 * @generated
 */
public class Learner extends CellOccupant {
	...
	public void initializeState() {
		...
		int incrementNextLearnerID = //increment
		getSocialResources().getNextLearnerID() + 1;
	...
	public void guidedGrowth() {
		...
		if (guide != null) {
			double potentialInfluence = //sub
			guide.getEfficacy() - getLearningInterest();

			if (potentialInfluence > 0) {
				double guideInfluence = //mul
				potentialInfluence
						* guide.getSocialResources().getGuideContributionRate();

				double performanceAddGuideInfluence = //add
				guide.getContributionTotal() + guideInfluence;

				double newInterestLevel = //add
				getLearningInterest() + guideInfluence;
	...
	public void pickGuide() {
		...
		if (newGuide != null) {
			((org.ascape.model.space.Graph) getSocialResources()
					...
					newGuide, true);
			int guideIDAddZero = //add
			newGuide.getGuideID() + 0;

			int incrementLearnerCount = //increment
			newGuide.getLearnerCount() + 1;
	...
	public void selfDirectedGrowth() {
		double potentialInterestGrowth = //sub
		getLearningInterest()
				- getSocialResources().getSelfContributionThreshold();

		if (potentialInterestGrowth > 0) {
			double selfInfluence = //mul
			potentialInterestGrowth
					* getSocialResources().getSelfContributionRate();

			double learningInterestAddSelfInfluence = //add
			getLearningInterest() + selfInfluence;

			double minimum_LearningInterestAddSelfInfluenceUnit_ = //min
			java.lang.Math.min(learningInterestAddSelfInfluence, 1)

			;

			double actualGrowth = //sub
			minimum_LearningInterestAddSelfInfluenceUnit_
					- getLearningInterest();

			setLearningInterest(minimum_LearningInterestAddSelfInfluenceUnit_);
			double selfContributionAddActualGrowth = //add
			getSelfContribution() + actualGrowth;
