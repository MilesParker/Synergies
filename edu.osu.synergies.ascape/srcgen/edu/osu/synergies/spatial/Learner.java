...
public class Learner extends CellOccupant {
	...
	public void initializeState() {
		...
		getSpatialResources().getNextLearnerID() + 1;
	...
	public void resourceEngagement() {
		if ((getBehavior() == BehaviorEnum.notInterested || getBehavior() == BehaviorEnum.seekingResource)) {
			...
			if (encounteredResource != null) {
				double learningInterestAddEfficacy = //add
				getLearningInterest() + encounteredResource.getEfficacy();
				...
				if (allLearnerInteractions != null) {
					if ((allLearnerInteractions.getResourceID() == encounteredResource
							.getResourceID() && allLearnerInteractions
							.getLearnerID() == getLearnerID())) {
						double newStrength = //increment
						allLearnerInteractions.getStrength() + 1;

						int incrementResourceEngagements = //increment
						getResourceEngagements() + 1;

						allLearnerInteractions.setStrength(newStrength);
						setResourceEngagements(incrementResourceEngagements);
						int incrementLearnerInteractions = //increment
						encounteredResource.getLearnerInteractions() + 1;
					...
					int resourceIDAddZero = //add
					encounteredResource.getResourceID() + 0;

					createAgents.setResourceID(resourceIDAddZero);
				}
				double minimum_LearningInterestAddEfficacyUnit_ = //min
				java.lang.Math.min(learningInterestAddEfficacy, 1)
		...
		if (getSpatialResources().getTimeSegment() == 0) {
			...
			if (targetResource != null) {
				if (getLearnerID() == targetResource.getLearnerID()) {
					double strengthMultiplySubsequentEngagementFactor = //mul
					targetResource.getStrength()
							* targetResource.getSpatialResources()
									.getSubsequentEngagementFactor();

					double randomUnit = //randomUnit
					getRandom().nextDouble();

					if (randomUnit < strengthMultiplySubsequentEngagementFactor) {
						setBehavior(BehaviorEnum.seekingResource);
						int resourceIDAddZeroCopy = //add
						targetResource.getResourceID() + 0;
	...
	public void findHome() {
		...
		if (homeLocationLocation != null) {
			...
			if (homeLocation != null) {
				int homeIDAddZero = //add
				homeLocation.getHomeID() + 0;
